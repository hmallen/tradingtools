...
class Display(threading.Thread):
    ...
    def __init__(self, master=None, trade_update_interval=1, analysis_update_interval=10, timeout_interval=120):
        self.root = master
    ...
    def create_widgets(self):
        ...
        update_status_result = self.update_status(mode='updating')
        logger.debug('update_status_result: ' + str(update_status_result))

        if update_status_result['success'] == False:
            logger.error('Error while updating status during start-up. Exiting.')
            sys.exit(1)
    ...
    def process_combobox_selections(self, update_managers=False):
        ...
        try:
            ...
            if update_managers == True:
                ...
                if self.binance_candles != None:
                    logger.info('Closing existing candles websocket.')
                    self.binance_candles.close()
    ...
    def update_status(self, mode):
        """
        Update status message/icon to reflect current conditions
        ...
        update_status_return = {'success': True}

        try:
            if mode == 'updating':
                self.variables['menu']['status'].set('Updating')
                self.widgets['menu']['variables']['status'].configure(bg=self.colors['bg']['updating'], relief=tk.SUNKEN)
            elif mode == 'ready':
            ...
            try:
                ...
                if self.gui_data_ready == False:
                    if self.trade_data_ready == True and self.analysis_data_ready == True:
                        #self.variables['menu']['status'].set('Ready')
                        #self.widgets['menu']['variables']['status'].configure(bg=self.colors['bg']['ready'], relief=tk.RAISED)
                        update_status_result = self.update_status(mode='ready')
                        logger.debug('update_status_result: ' + str(update_status_result))
                ...
                for update in self.update_last:
                    #logger.debug('update: ' + update)

                    update_time = self.update_last[update]
                    #logger.debug('update_time: ' + str(update_time))

                    if update_time != None:
                        update_delta = time.time() - update_time
                        #logger.debug('update_delta: ' + str(update_delta))

                        if update_delta > self.timeout_interval:
                            logger.warning(update.capitalize() + ' database values are not up-to-date. Confirm that flowmeter core is running.')
                            update_status_result = self.update_status(mode='error')
                            status_warning = True

                    else:
                        logger.warning('No value for ' + update.capitalize() + ' last update time.')
                        status_warning = True

                if self.variables['menu']['status'].get() != 'Ready' and status_warning == False:
                    logger.debug('Database values up-to-date. Resetting status message.')
                    update_status_result = self.update_status(mode='ready')

                    if update_status_result['success'] == False:
                        logger.error('Error while resetting status message.')
    ...
    def update_trade_values(self):
        ...
        try:
            ...
            if aggregate_result['ok'] == 1:
                ...
                self.update_last['trade'] = trade_last['trade_time'] / 1000
    ...
    def update_analysis_values(self):
        ...
        try:
            ...
            if aggregate_result['ok'] == 1:
                ...
                self.update_last['analysis'] = analysis_last['updated']

                logger.info(datetime.datetime.fromtimestamp(self.update_last['analysis']).isoformat(timespec='seconds', sep=' ') +
                            ' - Flow Differential: ' + "{:.2f}".format(flow_differential))
