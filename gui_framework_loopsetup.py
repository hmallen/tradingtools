...
class Display(threading.Thread):
    ...
    def create_widgets(self):
        ...
        width_trade, height_trade = 2, 6;
        self.widget_grid['trade'] = [[None for x in range(width_trade)] for y in range(height_trade)]

        ## Create widgets ##
        # Trade Title
        self.widgets['trade']['titles']['main'] = tk.Label(self.root, text='Last Trade')
        self.widget_grid['trade'][0][0] = ('titles', 'main', True)
        ...
        self.widget_grid['trade'][1][0] = ('text', 'price', False)
        ...
        self.widget_grid['trade'][1][1] = ('variables', 'price', False)
        ...
        self.widget_grid['trade'][4][0] = ('variables', 'status', True)
        ...
        #self.widget_grid['trade'][7][0] = ('text', 'update_pending', True)
    ...
    def update_trade_display(self, data):
        ...
        try:
            ...
            self.variables['trade']['price'].set(round(data['price'], 8))
            self.variables['trade']['quantity'].set(round(data['quantity'], 2))
            self.variables['trade']['amount'].set(round(data['price'] * data['quantity'], 2))
            #side_current = data['side'].upper()
            #self.variables['trade']['side'].set(side_current)
            if data['side'] == 'buy':
                #self.widgets['trade']['variables']['side'].config(image=self.images['trade']['arrow_buy'],
                                                                  #bg=self.colors['trade']['bg']['buy'])
                self.widgets['trade']['variables']['price'].config(image=self.images['trade']['arrow_buy'])
            else:
                #self.widgets['trade']['variables']['side'].config(image=self.images['trade']['arrow_sell'],
                                                                  #bg=self.colors['trade']['bg']['sell'])
                self.widgets['trade']['variables']['price'].config(image=self.images['trade']['arrow_sell'])
            #self.widgets['trade']['variables']['side'].update()
